...
def gibbs_sampler_dna(peptides_list:list, alphabet:list, NTscoring:np.array, GC_content:float, beta=0.0, iters_per_point=6, seed=1, T_i=1.0, T_f=0.0001, T_steps=5, sequence_weighting=False):
...
dat_all = pd.read_csv(data_dir + "/gene_seqs.csv", index_col=0)
pep_df = dat_all.loc[dat_all.imodulon == "AraC", "seq"]
peptides_list = pep_df.to_list()

peptides_file = "/gyrA.txt" #args.peptides_file
# peptides_list = np.loadtxt(data_dir + peptides_file, dtype=str).tolist()
